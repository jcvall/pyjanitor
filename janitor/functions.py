...
@pf.register_dataframe_method
def tally(df: pd.DataFrame(), target_column: str, new_column_name: str, include_na: bool):
    """
    Add a new count column from target column
    
    Count of target column's unique values (can be qualitive or quantative) returning
    a float value in a new specified column name.
    ...
    data = pd.DataFrame({'a': [1, 2, None, 4,5,5,7],
                         'b': [5.0, None, 7.0, 8.0,9.0,1.0,3.0]
                         'c': ['Bats','Cats','Bats','Cats','Bats','Bats','Bats']})
    
    Dont count NAs
    df = jn.tally( df: df, target_column: 'c', new_column_name: 'count_of_c', include_na: False )
    
    Include count of NAs
    df = jn.tally( df: df, target_column: 'c', new_column_name: 'count_of_c', include_na: True )
    ...
    Dont count NAs
    df = df.tally(target_column: 'c', new_column_name: 'count_of_c', include_na: False )
    
    Include count of NAs
    df = df.tally(target_column: 'c', new_column_name: 'count_of_c', include_na: True )
    
    :param df: A pandas DataFrame.
    :param target_column: The column on which the unique values will be counted
    :param new_column_name: Name of the new column
    :param include_na: False - returns NA where target column has NA values, True - returns
    sum count of NAs in target column 
    
    """  
    
    if include_na == True:
        df[new_column_name] = df.groupby([target_column])[target_column].transform('count')
